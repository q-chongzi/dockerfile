...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="content">
		<xsl:if test="parameter.cludove/report_time != ''">
			<xsl:text>BUDDLE_ID != 'infoinge' and </xsl:text>
		</xsl:if>
		<xsl:choose>
			<xsl:when test="count(parameter.cludove/buddle_name) = 0">
				<xsl:if test="parameter.cludove/report_time = 0">
					...
					<xsl:text disable-output-escaping="yes"> and RECORD_HOUR != 99 </xsl:text>
				</xsl:if>
				<xsl:if test="parameter.cludove/report_time = 1">
					...
					<xsl:text> and RECORD_MONTH = </xsl:text>
					<xsl:value-of select="parameter.cludove/month" />
					<xsl:text disable-output-escaping="yes"> and RECORD_DAY != 99 </xsl:text>
					<xsl:text> and RECORD_HOUR = 99 </xsl:text>
				</xsl:if>
				<xsl:if test="parameter.cludove/report_time = 2">
					...
					<xsl:text>' and RECORD_YEAR = </xsl:text>
					<xsl:value-of select="parameter.cludove/year" />
					<xsl:text disable-output-escaping="yes"> and RECORD_MONTH != 99 </xsl:text>
					<xsl:text> and RECORD_DAY = 99 </xsl:text>
					<xsl:text> and RECORD_HOUR = 99 </xsl:text>
				</xsl:if>
				<xsl:if test="parameter.cludove/report_time = 3">
					<xsl:text>JOURNAL_TYPE='</xsl:text>
					<xsl:value-of select="parameter.cludove/journal_type" />
					<xsl:text disable-output-escaping="yes">' and RECORD_YEAR != 99 </xsl:text>
					<xsl:text> and RECORD_MONTH = 99 </xsl:text>
					<xsl:text> and RECORD_DAY = 99 </xsl:text>
					<xsl:text> and RECORD_HOUR = 99 </xsl:text>
				</xsl:if>
			</xsl:when>
