            ...
            <DECOMPOSING_CODE>
                <xsl:choose>
                    <xsl:when test="count(cludove.crest.spruce.resource_decomposing[1]/record.crest) > 0">
                        <xsl:value-of select="cludove.crest.spruce.resource_decomposing[1]/record.crest/code"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="cludove.crest.spruce.resource_decomposing[2]/record.crest/code"/>
                    </xsl:otherwise>
                </xsl:choose>
            </DECOMPOSING_CODE>
            <REG_STATUS>
                <xsl:text disable-output-escaping="yes">regist</xsl:text>
            </REG_STATUS>
            <xsl:if test="count(cludove.crest.spruce.resource_connection/record.crest) > 0">
                <CONNECTION_CODE>
