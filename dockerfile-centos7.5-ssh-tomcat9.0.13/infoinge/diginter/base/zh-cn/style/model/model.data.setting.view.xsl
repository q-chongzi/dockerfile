    ...
    <xsl:template match="content">
            ...
            <xsl:choose>
                <xsl:when test="count(cludove.crest.spruce.data[1]/record.crest) > 0">
                    ...
                    <div class="span_edit span_data_edit" title="新增目录数据">
                        ...
                        <xsl:attribute name="onclick">$('#code_data').val('');$('#data_type_hinge').show();$('#data_type_hinge_label').show();$(".set_data_name").html("目录数据-新增");$('#data_operation_setting_this').val('');$('#data_position').val('<xsl:value-of select="$position_new + 1"/>'); $(".span_data_edit").hide(); $('#connection_code_data').val(''); $('#data_name').focus();  $('input:radio[name="data_select"]').attr('checked',false);if('<xsl:value-of select="$position"/>'=='100'){}else{$('#data_position_setting').val('<xsl:value-of select="$position_new + 1"/>')};$('#data_operation_migrate_container').css('display','none');$('#data_operation_container').css('display','block');initDataOperation();$('#operation_by_db').val('');$("#data_position").removeAttr('disabled','disabled'); if($("#data_catalog_type").val().length == 0){$('input:radio[name="data_type_select"]').removeAttr('disabled')};if ( $("#data_catalog_type").val() == 'synchronization' ||   $("#data_catalog_type").val() == 'migration') {$('#data_position').spinner({max: 100,min: 1,step: 1});$('#data_exchange_container').css('display','none')};<!--$("#data_operation").find("option[value='search']").attr("selected",true);-->if($('#data_operation_setting').val()=='mysql_migrate' || $('#data_operation_setting').val()=='oracle_migr' || $('#data_operation_setting').val()=='sql_migr' || $('#data_operation_setting').val()=='mysql_sync' || $('#data_operation_setting').val()=='oracle_sync' || $('#data_operation_setting').val()=='sql_sync'){$("#data_operation").find("option[value='search']").css("display","none");$('input:radio[name="data_type_select"]').attr('disabled','disabled');}newModelAddButton();$('#repeat_field_container').show(); </xsl:attribute>
                    </div>
                </xsl:when>
                <xsl:otherwise>
        ...
        <form class="form-horizontal" role="form" id="validate_data_setting">
            ...
            <p class="design-rad">
                ...
                <!--onDataConnectionIdChange();$("#data_connection_id").removeAttr('onchange');-->
                ...
                <label id="data_type_hinge_label" for="data_type_hinge">
            ...
            <p class="list">
                ...
                <div style="position: relative;">
                    <span style="color:red; display:none;position: absolute;top: -52px;left: 60px;" id="hinge_content_block">
            ...
            <div>
                ...
                <div id="data_div_btn" class="configure-fr-side-bottom">
                    ...
                    <p class="configure-fr-side-bottom">
                        <xsl:if test="count(cludove.crest.spruce.data_exchange/record.crest[migr_position !='' or sync_position !='']) = 0">
                            <input type="button" href="javascript:void(0)" value="提交" class="submit_btn sub">
                                <xsl:attribute name="onclick">confirmModelModifyDataSetting()</xsl:attribute>
                            </input>
                        ...
                        <xsl:if test="cludove.crest.spruce.data[1]/record.crest/code != ''">
                            <xsl:choose>
                                <xsl:when test="cludove.crest.spruce.data[1]/record.crest/operation !=  'search' and cludove.crest.spruce.data[1]/record.crest/operation !=  'build' and cludove.crest.spruce.data[1]/record.crest/operation !=  'modify' and cludove.crest.spruce.data[1]/record.crest/operation !=  'remove' and cludove.crest.spruce.data[1]/record.crest/operation !=  'upload' and cludove.crest.spruce.data[1]/record.crest/operation !=  'download'"/>
                                <xsl:otherwise>
                                    <input type="button" href="javascript:void(0)" class="but" onclick="processModelDataRemove()" value="删除"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:if>
                    ...
                    <input type="hidden" name="data_catalog_type" id="data_catalog_code">
                    ...
                    <input type="hidden" id="data_name">
                        <xsl:attribute name="value"><xsl:value-of select="cludove.crest.spruce.data[1]/record.crest/name"/></xsl:attribute>
                    </input>
                    <!--用做库表关联-->
                    ...
                    <input type="hidden" name="data_max" id="data_max">
