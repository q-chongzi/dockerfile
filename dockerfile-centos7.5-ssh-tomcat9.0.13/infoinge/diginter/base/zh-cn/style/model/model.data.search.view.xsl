...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="content">
        ...
        <xsl:if test="count(cludove.crest.spruce.data_metadata/record.crest[data_id = $data_id]) > 0">
            ...
            <xsl:value-of select="$data_code"/>
            <xsl:text>",db_number:"</xsl:text>
            <xsl:value-of select="count(/content/cludove.crest.spruce.data/record.crest[type = 'DB'])"/>
        ...
        <!--循环取出模型设计的图表，并进行左右分类-->
        <xsl:for-each select="cludove.crest.spruce.data/record.crest[(operation != 'build' and operation != 'modify' and operation != 'remove' and operation != 'upload' and operation != 'download')]">
            ...
            <xsl:text>",db_number:"</xsl:text>
            <xsl:value-of select="count(/content/cludove.crest.spruce.data/record.crest[type = 'DB'])"/>
            <xsl:text>",metadata_select:"</xsl:text>
            ...
            <xsl:choose>
                <xsl:when test="count(/content/metadata[@id = $metadata_select]/record.crest[data_direct]) >0">
                    <xsl:for-each select="/content/metadata[@id = $metadata_select]/record.crest[data_direct='output']">  <!--循环属性为id=‘’的数据-->
                ...
                <xsl:otherwise>
                    <xsl:for-each select="/content/metadata[@id = $metadata_select]/record.crest">  <!--循环属性为id=‘’的数据-->
                        <xsl:variable name="field_name">
                            <xsl:value-of select="name"/>
                        </xsl:variable>
                        <xsl:if test="position() > 1">
        ...
        <xsl:text>],[</xsl:text>
        <xsl:for-each select="cludove.crest.spruce.data/record.crest[(operation = 'build')]">
            <xsl:variable name="metadata_select">
                <xsl:choose>
                    <xsl:when test="type = 'Hinge'">
                        <xsl:value-of select="connection_id"/>
            ...
            <xsl:choose>
                <xsl:when test="count(/content/metadata[@id = $metadata_select]/record.crest[data_direct]) > 0">
                    <xsl:for-each select="/content/metadata[@id = $metadata_select]/record.crest[data_direct='input']">  <!--循环属性为id=‘’的数据-->
                        <xsl:variable name="field_name">
                            <xsl:value-of select="name"/>
                        </xsl:variable>
                        <xsl:if test="position() > 1">
                        ...
                        <xsl:choose>
                            <xsl:when
                                    test="count(/content/cludove.crest.spruce.field_label/record.crest[field_name = $field_name and resource_name = $resource_name and identifier = $identifier]) > 0">
                                <xsl:value-of
                                        select="/content/cludove.crest.spruce.field_label/record.crest[field_name = $field_name and resource_name = $resource_name and identifier = $identifier]/field_label"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="remark"/>
                            </xsl:otherwise>
        ...
        <xsl:for-each select="/content/cludove.crest.spruce.data_mapping/record.crest">
            ...
            <xsl:variable name="r_type">
                <xsl:value-of select="/content/cludove.crest.spruce.data/record.crest[code = $r_data_code]/type"/>
            </xsl:variable>
            <xsl:choose>
                <xsl:when test="$r_type='Class' or $r_type='WS'">
