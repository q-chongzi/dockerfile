...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="content">
        <html>
            ...
            <body>
                <div>
                    <button onclick="processCatalogParameterEdit()">保存</button>
                    <br/>
                    <br/>
                    <script id="catalog_parameter_requirement" type="text/plain" style="width:100%; height:600px;">
                        <xsl:value-of select="cludove.crest.spruce.catalog_parameter/record.crest/requirement"/>
                        <xsl:text disable-output-escaping="no"> </xsl:text>
                    </script>
                </div>
                ...
                <input type="hidden" name="connection_code" id="connection_code_catalog_parameter">
                    <xsl:attribute name="value">
                        <xsl:choose>
                            <xsl:when test="cludove.crest.spruce.catalog_parameter/record.crest/connection_code != ''">
                                <xsl:value-of select="cludove.crest.spruce.catalog_parameter/record.crest/connection_code"/>
                            </xsl:when>
                            <xsl:otherwise>
            ...
            <script type="text/javascript" src="diginter/base/zh-cn/js/navigate.js">
                <xsl:text disable-output-escaping="no"> </xsl:text>
            </script>
            ...
            <script type="text/javascript">
                $(document).ready(function() {setupHttpHeader(); var base64 = new Base64(); $('#catalog_parameter_requirement').text(base64.decode($('#catalog_parameter_requirement').text())); var ue = UE.getEditor('catalog_parameter_requirement');});
            </script>
        </html>
    </xsl:template>
</xsl:stylesheet>
