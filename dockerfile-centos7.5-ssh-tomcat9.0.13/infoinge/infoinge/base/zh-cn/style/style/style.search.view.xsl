			...
			<table>
				...
				<xsl:for-each select="/content/cludove.crest.spruce.style/record.crest">
					<tr>
						...
						<td>
							<input type="radio" name="style_select">
								<xsl:attribute name="onclick">showStyleSetting('<xsl:value-of select="code" />')</xsl:attribute>
								...
								<xsl:if test="code != 'spruce-sty-1000000101'">
									<a class="a_dele" href="javascript:void(0)" title="停用">
										<img src="infoinge/base/zh-cn/img/dele.png">
											...
											<xsl:attribute name="onclick">if (confirm("确认停用 风格 <xsl:value-of select="class" />/<xsl:value-of select="name" />  吗？")) {$.request(httpHeader, '', new Array('buddle.cludove', 'catalog.cludove', 'style_record_code', 'is_active', 'connection_code'), new Array('infoinge', 'style.remove.process', '<xsl:value-of select="code" />', 'false', '<xsl:value-of select="connection_code" />'), new Array(), new Array(), 'text',null, null, 'onRecordRemoveCallBack', 'normalErrorCallback');}</xsl:attribute>
										</img>
									</a>
								</xsl:if>
