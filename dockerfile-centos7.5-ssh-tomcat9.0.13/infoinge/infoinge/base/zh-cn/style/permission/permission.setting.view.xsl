							...
							<xsl:if test="$permission_search_setting != ''">
								<label class="radio i-checks checkbox">
									<input type="checkbox" name="check_permission_search">
										...
										<xsl:choose>
											<xsl:when test="count(/content/cludove.crest.spruce.permission_control/record.crest[buddle_id = 'infoinge' and catalog_id = 'permission.setting.process']) = 0 and /content/parameter.cludove/account.cludove != 'admin'">
												<xsl:attribute name="disabled">disabled</xsl:attribute>
											</xsl:when>
											<xsl:when test="$permission_search_setted = '' or contains($permission_search_setted, $owner_id)">
												<xsl:attribute name="onclick">if($('#permission_check_number').val() == '0'){$("#now_remove").show();$('#ajax_loader_remove').show();}$.request(httpHeader, '', new Array('buddle.cludove', 'catalog.cludove', 'buddle_id', 'is_active', 'data_name', 'data_operation', 'owner_type', 'owner_code'), new Array('infoinge', 'permission.setting.process', '<xsl:value-of select="/content/parameter.cludove/buddle_id"/>', String(this.checked) , '<xsl:value-of select="$data_name"/>', 'search', '<xsl:value-of select="/content/parameter.cludove/owner_type"/>', '<xsl:value-of select="/content/parameter.cludove/owner_code"/>'), new Array(), new Array(), 'text', null, null, 'permissionSuccessCallback', 'normalErrorCallback');</xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<xsl:attribute name="disabled">disabled</xsl:attribute>
											</xsl:otherwise>
