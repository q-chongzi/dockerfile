...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="content">
        <!--向总线提交参数-->
        <xsl:choose>
            <xsl:when test="count(cludove.crest.spruce.resource_connection[3]/record.crest) = 0  or cludove.crest.spruce.resource_connection[1]/record.crest[1]/is_locked = 'true'"/>
            <xsl:otherwise>
                <parameter>
                    <buddle.cludove>
